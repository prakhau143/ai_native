    ...
    <section id="audit" className="py-20 px-4 sm:px-6">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="text-center mb-12">
          <div
            className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-medium text-purple-600 dark:text-purple-300 mb-4"
          ...
          {tools.map((entry, idx) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wider">
                  Tool #{idx + 1}
                </span>
                {tools.length > 1 && (
                  <button
                    onClick={() => removeTool(entry.id)}
                    className="flex items-center gap-1 text-xs text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-colors px-2 py-1 rounded-lg hover:bg-red-400/10"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <div className="lg:col-span-1">
                  ...
                  <div className="relative">
                    ...
                    >
                      {TOOLS.map((t) => (
                        <option key={t.id} value={t.id} style={{ background: "var(--card-solid)" }}>
                          {t.name}
                        </option>
                      ))}
                    </select>
                    <ChevronDown className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 dark:text-slate-500" />
                  </div>
                </div>
                  ...
                  <div className="relative">
                    ...
                    >
                      {selectedPlans(entry.toolId).map((p) => (
                        <option key={p.name} value={p.name} style={{ background: "var(--card-solid)" }}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      {USE_CASES.map((u) => (
                        <option key={u.value} value={u.value} style={{ background: "var(--card-solid)" }}>
                          {u.label}
                        </option>
                      ))}
                    </select>
                ...
                <div>
                  <label className="block text-xs font-medium text-cyan-600 dark:text-cyan-400 mb-1.5 uppercase tracking-wide">
                    <span className="flex items-center gap-1">
                      <DollarSign className="h-3 w-3" />Per seat $
                    </span>
                  </label>
                  <input
                    type="number"
                    ...
                    className="futuristic-input w-full rounded-xl px-3 py-2.5 text-sm"
                    placeholder="0"
                  />
                </div>
              ...
              <div className="mt-3 pt-3 flex justify-end" style={{ borderTop: "1px solid var(--card-border)" }}>
                <span className="text-xs text-gray-500 dark:text-slate-500">
                  ...
                  <span className="text-cyan-600 dark:text-cyan-300 font-semibold">
                    ${(entry.monthlySpend * entry.seats).toLocaleString()}/mo
                  </span>
                </span>
              </div>
            </div>
        ...
        <button
          onClick={addTool}
          className="mb-8 w-full flex items-center justify-center gap-2 rounded-2xl py-3 text-sm font-medium text-gray-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-300 transition-all duration-200"
        ...
        >
          <div>
            <p className="text-sm text-gray-600 dark:text-slate-500 mb-0.5">Current monthly spend</p>
            <p
            ...
            >
              ...
              <span className="text-base font-normal text-gray-500 dark:text-slate-500">/mo</span>
            </p>
          ...
          <button
            onClick={() => tools.length > 0 && onSubmit(tools)}
